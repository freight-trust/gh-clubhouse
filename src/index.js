...
export async function clubhouseStoryToGithubIssue(
  clubhouseStoryURL,
  githubRepoURL,
  options = {}
) {
  ...
  const unsavedIssueComments = _presentClubhouseComments(
    story.comments,
    clubhouseUsersById
  );
  ...
  await Bluebird.each(unsavedIssueComments, (comment) =>
    createIssueComment(options.githubToken, owner, repo, issue.number, comment)
  );
...
export async function githubIssueToClubhouseStory(options) {
  ...
  const stateId = {
    open: clubhouseWorkflows[0].states.find(
      (state) => state.type === "unstarted"
    ).id,
    done: clubhouseWorkflows[0].states.find((state) => state.type === "done")
      .id,
  };
  // log("stateId", stateId)
  ...
  const project = projects.find(
    (project) => project.name === options.clubhouseProject
  );

  if (!project) {
    throw new Error(
      `The '${options.clubhouseProject}' project wasn't found in your Clubhouse`
    );
  }
  ...
  for (const issue of issues) {
    ...
    if (existingStory) {
      ...
      const issueComments = await getCommentsForIssue(
        options.githubToken,
        owner,
        repo,
        issue.number
      );
      const issueLabels = await getLabelsForIssue(
        options.githubToken,
        owner,
        repo,
        issue.number
      ...
      const unsavedStory = _issueToStory(
...
function _presentClubhouseComments(comments, clubhouseUsersById) {
  return comments.map((comment) => {
    const user = clubhouseUsersById[comment.author_id] || {
