...
test("util/apiFetch", (t) => {
  t.test("apiFetch", (tt) => {
    ...
    tt.test("returns a Promise of data if successful", (tt) => {
      ...
      const expectedResults = [{ title: "first" }, { title: "second" }];
      nock("https://api.example.com")
        .get("/yet/another/path")
        .reply(200, JSON.stringify(expectedResults));

      return apiFetch("https://api.example.com/yet/another/path").then((data) =>
        tt.deepEqual(data, expectedResults)
      );
    });
  ...
  t.test("apiFetchAllPages", (tt) => {
    tt.test("returns a Promise of all pages of data if successful", (ttt) => {
      ...
      const page1Data = [{ title: "first" }, { title: "second" }];
      const page2Data = [{ title: "third" }, { title: "fourth" }];
      const page3Data = [{ title: "fifth" }, { title: "sixth" }];
